...
가입할 때 정한 사용자명이 앱 주소가 됩니다. 예를 들어 사용자명이 `myname`이고 앱 이름을 `outcasts2026`으로 정하면 앱 주소는 `https://myname.github.io/outcasts2026/` 이 됩니다.
...
파일을 GitHub에 올리는 데 필요한 도구입니다.

1. https://git-scm.com/download/mac 접속
2. 다운로드된 파일 실행 후 설치
3. 설치 확인: 터미널에서 `git --version` 입력 → 버전 숫자가 나오면 성공

> **터미널 여는 법**: 키보드에서 `Cmd+Space` → `터미널` 검색 → 엔터
...
1. https://code.visualstudio.com 에서 **Download for Mac** 클릭 후 설치
2. VS Code 실행
3. 왼쪽 아이콘 중 네모 4개 모양(Extensions) 클릭, 또는 `Cmd+Shift+X`
4. 검색창에 `Claude Code` 입력
5. **Claude Code** (Anthropic 제작) 옆 **Install** 클릭
6. 설치 후 상단 메뉴 **Terminal → New Terminal** 클릭
7. 하단에 터미널 창이 열리면 `claude` 입력 후 엔터

#### 방법 B — 터미널 앱 직접 사용

1. https://claude.ai/code 에서 다운로드 후 설치
2. 터미널(`Cmd+Space` → `터미널`)에서 `claude` 입력 후 엔터
...
터미널에서 아래 명령을 **한 줄씩** 입력하세요. 각 줄 입력 후 엔터를 누릅니다.
